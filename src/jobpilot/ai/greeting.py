...
2. **只用一个最相关的产品**打钩子（按 use_for 判断），绝不同时讲两个产品
...
## 输出
只输出钩子句正文，第一人称，**只讲一个产品、整体不超过 100 字**（自我介绍已占约 145 字，全文要压在 300 字内）。
**不要**句尾标点（后面会自动接"，希望能进一步沟通。"）。
不要任何解释、标题或引号。

长度与风格范例（仿照这个长度，不要更长）：
做 MusiClaw 时我自建了 eval 框架，用 YAML 定义测试集跑 A/B 实验评测 prompt，看到你们要做数字人评测和 vibe-coding，这套方法能直接用上、帮团队建可量化的迭代标准
"""
